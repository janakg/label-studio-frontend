  ...
  return (
    <Tree
      ...
      onDrop={({ node, dragNode, dropPosition, dropToGap }) => {
        ...
        const oldIndex = parseInt(dragNode.props.pos.split("-").pop());
        const dropIndex = parseInt(node.props.pos.split("-").pop());
        const noNodes = regionStore.regions.length;        
        let newIndex = (oldIndex <= dropIndex)? dropIndex : dropIndex + 1;

        if(-1 === dropPosition || noNodes <= dropPosition) newIndex = dropIndex;
        ...
        const regionIds = reorderArray({ newIndex, oldIndex }, regionStore.sortedRegions.map(r => r.id));

        regionStore.unhighlightAll();
        console.log(regionStore.regions);
        regionStore.sortRegionsByScore(regionIds);
